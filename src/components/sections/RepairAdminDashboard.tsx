...
const IN_PROGRESS_STATUSES = [
  "received", "diagnosing", "in_repair", "repaired", "quality_check",
  "in_transit_to_shop", "label_sent", "approved",
];
...
export default function RepairAdminDashboard({ shop, slug }: Props) {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/shop-admin/repair-orders", { credentials: "include" });
      if (res.status === 401) {
  ...
  const handleLogout = async () => {
    await fetch("/api/shop-admin/auth", { method: "DELETE", credentials: "include" });
    setAuthenticated(false);
    setOrders([]);
  };
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-[420px]">
          ...
          <p className="text-center text-xs text-slate-600 mt-6">Nur für autorisierte Mitarbeiter</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center shadow-sm">
                <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17l-5.384 3.18 1.029-5.997L1.68 7.246l6.023-.875L11.42.854l2.692 5.517 6.023.875-4.385 4.107 1.029 5.998z" />
                </svg>
              </div>
              <div>
                <h1 className="text-sm font-bold text-gray-900 leading-none">{shop.name}</h1>
                <p className="text-[11px] text-gray-400 leading-none mt-0.5">Reparatur-Dashboard</p>
              </div>
            </div>
          ...
          <div className="flex items-center bg-white rounded-xl border border-gray-200 p-1 gap-0.5 overflow-x-auto">
            {FILTER_TABS.map((tab) => {
              const count =
                ...
                tab.key === "estimate" ? stats.estimateOpen :
                stats.completed;
              return (
                <button
                  key={tab.key}
          ...
          <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-gray-100">
            ...
            {orders.length === 0 && (
              <p className="text-xs text-gray-400 mt-3 max-w-sm text-center">
                Reparatur-Anfragen erscheinen hier, sobald Kunden das Formular unter{" "}
                <a href={`/${slug}/repair-request`} className="font-medium text-indigo-600 hover:underline">
                  /{slug}/repair-request
                </a>{" "}
                absenden.
              </p>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {order.repair_types.length > 0 ? (
                          order.repair_types.slice(0, 3).map((r) => (
                            <span key={r} className="px-2 py-0.5 bg-gray-50 text-gray-600 text-[11px] font-medium rounded-md">
                              {REPAIR_LABELS[r] || r}
                            </span>
                          ))
                        ) : (
                      ...
                      <div className="border-t border-gray-100 bg-gray-50/50 rounded-b-xl">
                        <div className="px-5 py-5">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                              ...
                              <div className="space-y-2 text-sm">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {order.repair_types.map((r) => (
                                    <span key={r} className="px-2 py-0.5 bg-white border border-gray-200 text-gray-700 text-[11px] font-medium rounded-md">
                                      {REPAIR_LABELS[r] || r}
                                    </span>
                                  ))}
                                </div>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="space-y-3 text-sm">
                                {carrier && (
                                  <div className="flex items-center gap-2">
                                    <span className={`px-2.5 py-1 rounded-md text-xs font-bold ${CARRIER_COLORS[carrier]?.bg || "bg-gray-200"} ${CARRIER_COLORS[carrier]?.text || "text-gray-800"}`}>
                                      {carrier}
                                    </span>
                                ...
                                {order.inbound_tracking && (
                                  <a href={order.inbound_tracking.tracking_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-100 rounded-lg text-xs text-blue-700 hover:bg-blue-100 transition-all">
                                ...
                                {order.outbound_tracking && (
                                  <a href={order.outbound_tracking.tracking_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-100 rounded-lg text-xs text-emerald-700 hover:bg-emerald-100 transition-all">
                                    <span className="font-bold">↑</span>
                                ...
                                {!order.inbound_tracking && !order.outbound_tracking && (
                                  <p className="text-gray-400 text-xs italic">Kein Tracking vorhanden</p>
                                )}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="mt-5 pt-4 border-t border-gray-100 flex flex-wrap gap-2" onClick={(e) => e.stopPropagation()}>
                            {order.status === "pending" && (
                              <button onClick={() => openModal("label", order)} className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 transition-all">
                            ...
                            {["received", "diagnosing"].includes(order.status) && (
                              <button onClick={() => openModal("estimate", order)} className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-orange-200 text-orange-700 text-xs font-medium rounded-lg hover:bg-orange-50 transition-all">
                                KV senden
                              </button>
                            )}
                            <button onClick={() => openModal("tracking", order)} className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-gray-700 text-xs font-medium rounded-lg hover:bg-gray-50 transition-all">
                              Tracking
                            </button>
                            <a
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5 max-h-56 overflow-y-auto">
                      {getNextStatuses(modalOrder.status).map((s) => (
                        <label key={s} className={`flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-all ${newStatus === s ? "border-indigo-300 bg-indigo-50/50 ring-1 ring-indigo-200" : "border-gray-100 hover:border-gray-200"}`}>
                ...
                <>
                  <div className="flex gap-2">
                    {(["inbound", "outbound"] as const).map((t) => (
                      ...
                      >
                        {t === "inbound" ? "📦 Hinweg → Werkstatt" : "📦 Rückweg → Kunde"}
                      </button>
                    ))}
                  </div>
                  <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {modalOrder.customer_address.street}, {modalOrder.customer_address.zip} {modalOrder.customer_address.city}
                      </p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
